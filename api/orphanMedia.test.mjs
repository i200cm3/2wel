...
test(
  'collectProjectMediaSrcs walks nested config',
  withPublicDir(() => {
...
test(
  'listOrphanProjectMedia keeps shared files, lists only unused on disk',
  withPublicDir(() => {
    const code = 'c-orphan'
...
test(
  'mediaSrcsFromConfigs unions draft and published',
  withPublicDir(() => {
    const code = 'c-union'
...
test(
  'purgeProjectMediaSrcs deletes orphans and rebuilds library manifest',
  withPublicDir(() => {
    const code = 'c-purge'
...
test(
  'listUnusedProjectMedia drops never-referenced files when keep is empty',
  withPublicDir(() => {
    const code = 'c-unused'
...
test(
  'listProjectMediaOnDisk walks library music and tts',
  withPublicDir(() => {
    const code = 'c-disk'
...
test(
  'removeProjectMedia deletes library music and tts under the project',
  withPublicDir(async () => {
    const code = 'c-remove'
