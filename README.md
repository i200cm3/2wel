...
Команда объекта: владелец в кабинете → **Команда** приглашает сотрудника по почте. Если аккаунт уже есть — человек сразу попадает в объект. Если нет — ссылка `/join?invite=…` (14 дней). Сотрудник работает с шаблонами, ссылками и голосом, без прав администратора системы и без смены тарифа, интеграций и состава команды.
